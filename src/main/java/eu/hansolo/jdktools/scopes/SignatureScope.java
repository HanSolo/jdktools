...
public enum SignatureScope implements Scope {
     ...
     */
    public static List<SignatureScope> getAsList() { return getAsList(false); }
    public static List<SignatureScope> getAsList(final boolean withoutNoneAndNotFound) {
        if (withoutNoneAndNotFound) {
            return Arrays.asList(values()).stream()
                         .filter(signatureScope -> signatureScope != SignatureScope.NOT_FOUND)
                         .toList();
        } else {
