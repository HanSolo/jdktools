...
public enum UsageScope implements Scope {
     ...
     */
    public static List<UsageScope> getAsList() { return getAsList(false); }
    public static List<UsageScope> getAsList(final boolean withoutNoneAndNotFound) {
        if (withoutNoneAndNotFound) {
            return Arrays.asList(values()).stream()
                         .filter(usageScope -> usageScope != UsageScope.NOT_FOUND)
                         .toList();
        } else {
